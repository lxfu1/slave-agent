...
- **Slash 命令** — `/notes`、`/history`、`/search`、`/compact`、`/cost`、`/plan` 等，`/help` 查看全部
- **Plan-Execute-Reflect 循环** — `/plan <目标>` 触发三阶段 Agent 循环：规划（任务拆解）、执行（按依赖顺序逐任务调用工具）、反思（只读工具复盘）；进度实时流式显示
- **任务持久化** — Agent 创建的任务存储在 SQLite 中，与会话绑定，`/resume` 后可继续
...
- **Web 搜索** — 内置 `WebSearch` 工具，基于 Brave Search；配置 `search.apiKey` 后 Agent 可自主检索互联网信息
- **工具结果缓存** — 只读工具结果缓存 30 秒，文件写入后自动失效
...
- **RunCommand 沙箱** — 启用 `permissions.sandbox.enabled: true` 后，子进程只继承白名单中的环境变量，API Key 等敏感变量不泄露给子进程
...
| `/plan <目标>` | 启动 Plan-Execute-Reflect Agent 循环 |
...
## Plan-Execute-Reflect 循环（`/plan`）

`/plan <目标>` 触发三阶段自主循环：

1. **规划** — Agent 调用 `CreateTask` 将目标拆解为 3–7 个任务，并通过 `blockedBy` 声明依赖关系。此阶段不执行任何其他操作。
2. **执行** — 按拓扑顺序执行任务（被依赖的任务先执行），每个任务独立运行完整的工具调用循环。
3. **反思** — 所有任务结束后，Agent 用只读工具复盘结果并输出总结。
...
| `WebSearch` | 通过 Brave Search 搜索互联网；需配置 `search.apiKey` | 只读 |
...
permissions:
  ...
  sandbox:
    enabled: false           # 为 true 时子进程只继承白名单中的环境变量
    allowed_env_vars:
      - PATH
      - HOME
      - LANG
...
# Web 搜索（可选 — Brave Search）
search:
  provider: brave
  api_key: "${BRAVE_API_KEY}"
  max_results: 5             # 每次查询返回结果数（1–10）
...
- **路径限制**：文件读取、写入、编辑、列表与搜索均基于真实路径校验，只允许访问当前工作目录或 profile 目录，符号链接越界同样会被拒绝
...
- **RunCommand 沙箱**：启用 `permissions.sandbox.enabled: true` 后，子进程只继承白名单中的环境变量，API Key 等敏感变量不会泄露给子进程
